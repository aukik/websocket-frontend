<script>
    ...
    let isThisPageAccessible=false
    // onMount(() => {
    ...
    const token = sessionStorage.getItem('password'); // Replace 'yourTokenKey' with the actual key you use for the JWT in session storage
    ...
    if (token) {
        try {
            ...
            if (Date.now() > expirationTime) {
                // Token is expired, delete it from session storage
                sessionStorage.removeItem('password'); // Replace 'yourTokenKey' with the actual key you use for the JWT
                alert('Your session has expired. Please login again.');
                
                console.log('Token expired. Deleted from session storage.');
            ...
            console.error('Error decoding or checking the token:', error);
            return true; // Consider it expired in case of an error
        }
    } else {
...
{:else}
    <div class="flex flex-col justify-center items-center w-[100vw] h-[100vh]">
        <h1 class="my-2">Please Login to access this page</h1>
        <button class="self-center my-2" on:click={()=>{window.location.href="/"}}>Sign In</button>
    </div>
